...
fuzz_dir = f"{script_dir}/results/fuzzing/{args.model_name}/baseline"
detect_dir = f"{script_dir}/results/detection/{args.model_name}/baseline"
...
async def main():
    ...
    for layer in range(n_layers - 1):  # Create a pipeline for each layer
        ...
        dataset = LatentDataset(
            raw_dir=f"interp/latents/{args.model_name.replace('-', '_')}/baseline/layers.{layer}",
            cfg=latent_cfg,
            modules=[module],
